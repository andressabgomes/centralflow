...
/**
 * Script para importar clientes em lote no CentralFlow
 * 
 * Uso:
 * node scripts/bulk-import-customers.js [URL_DO_WORKER]
 * 
 * Exemplo:
 * node scripts/bulk-import-customers.js https://019911ce-cd37-7321-803c-5c193c44d1a9.andressagomes-adm.workers.dev
 */
...
const dataFile = path.join(__dirname, '..', 'bulk-customers.json');

console.log('🚀 CentralFlow - Importação de Clientes em Lote');
console.log('================================================');
...
if (!fs.existsSync(dataFile)) {
  console.error('❌ Erro: Arquivo bulk-customers.json não encontrado!');
...
try {
  ...
  console.log(`📊 Encontrados ${customers.length} clientes para importar`);
} catch (error) {
...
async function importCustomers() {
  ...
  for (let i = 0; i < customers.length; i++) {
    ...
    console.log(`[${customerNumber}/${customers.length}] Importando: ${customer.name}`);
  ...
  if (results.errors.length > 0) {
    ...
    results.errors.forEach((error, index) => {
      console.log(`  ${index + 1}. ${error.customer}: ${error.error}`);
    });
  }
  ...
  if (results.failed === 0) {
    console.log('🎊 Todos os clientes foram importados com sucesso!');
  } else {
